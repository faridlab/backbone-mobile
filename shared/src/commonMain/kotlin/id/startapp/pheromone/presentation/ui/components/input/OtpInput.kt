...
import id.startapp.pheromone.presentation.ui.theme.PheromoneBlueTint
 ...
 * Pheromone OTP Input
 *
 * 6-box OTP input with auto-advance following Pheromone design system.
 *
 * Specs:
...
private fun OtpBox(
    ...
    OutlinedTextField(
        ...
        modifier = Modifier
            .width(PheromoneDimensions.OtpBoxSize.dp)
            .height(PheromoneDimensions.OtpBoxSize.dp)
            .focusRequester(focusRequester)
        ...
        colors = OutlinedTextFieldDefaults.colors(
            ...
            focusedContainerColor = if (isError) ErrorRed.copy(alpha = 0.1f) else PheromoneBlueTint,
            unfocusedContainerColor = if (value.isNotEmpty()) Color.Transparent else Color.Transparent,
            cursorColor = if (isError) ErrorRed else PheromoneBlue
        ),
...
fun OtpInputWithTimer(
    ...
    modifier: Modifier = Modifier,
    onResend: () -> Unit,
    countdownSeconds: Int = PheromoneDimensions.OtpCountdownSeconds,
    error: String? = null,
    enabled: Boolean = true,
    onComplete: (() -> Unit)? = null
) {
    ...
    Column(
        ...
        OtpInput(
            ...
            length = PheromoneDimensions.OtpLength,
            error = error,
            enabled = enabled,
    ...
    Row(
        ...
        if (remainingSeconds > 0) {
            ...
            Text(
                text = "Resend",
                style = MaterialTheme.typography.bodyMedium,
                color = if (enabled) PheromoneBlue else SurfaceGray300,
