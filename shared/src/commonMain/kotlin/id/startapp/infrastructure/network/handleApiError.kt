...
suspend inline fun <T> apiCall(
...
): id.startapp.domain.types.Result<T> = try {
    val data = block()
    id.startapp.domain.types.Result.Success(data)
} catch (e: IOException) {
    CrashReporter.addBreadcrumb("network", "Connectivity error", mapOf("error" to (e.message ?: "unknown")))
    id.startapp.domain.types.Result.Error(NetworkError.ConnectivityError(e))
    ...
    id.startapp.domain.types.Result.Error(e)
} catch (e: Exception) {
    CrashReporter.captureException(e)
    id.startapp.domain.types.Result.Error(NetworkError.UnknownError(e))
}
