...
import id.startapp.domain.types.ConnectivityException
import id.startapp.domain.types.TimeoutException
...
fun Throwable.isRetryable(): Boolean {
    return when (this) {
        ...
        is id.startapp.domain.types.NetworkError -> when (this) {
            is id.startapp.domain.types.NetworkError.ConnectivityError -> true
            is id.startapp.domain.types.NetworkError.TimeoutError -> true
            is id.startapp.domain.types.NetworkError.ServerError -> {
...
suspend fun <T> retryWithBackoffResult(
    ...
    return when (result) {
        is RetryResult.Success -> {
            id.startapp.domain.types.Result.Success(result.value)
        }
        is RetryResult.Failure -> {
            val error = result.error
            when (error) {
                is id.startapp.domain.types.NetworkError -> {
                    id.startapp.domain.types.Result.Error(error)
                }
                is ConnectivityException -> {
                    id.startapp.domain.types.Result.Error(
                        id.startapp.domain.types.NetworkError.ConnectivityError(error)
                    )
                }
                is TimeoutException -> {
                    id.startapp.domain.types.Result.Error(
                        id.startapp.domain.types.NetworkError.TimeoutError(error)
                    )
                }
                is IOException -> {
                    id.startapp.domain.types.Result.Error(
                    ...
                    id.startapp.domain.types.Result.Error(
                        id.startapp.domain.types.NetworkError.UnknownError(error)
                    )
                }
            }
        }
